		  ...
		  case STAT_SUCCESS:
				...
				if (pdu->errstat == SNMP_ERR_NOERROR) {
				  ...
				  if (vp) snprint_objid(buf, sizeof(buf), vp->name, vp->name_length);
					...
					reportService->insertReport(id, MSG_ERR, snmp_errstring(pdu->errstat));
				}
				return 1;
		  case STAT_TIMEOUT:
				reportService -> insertReport(id, MSG_RECV, "Timeout");
				return 0;
		  case STAT_ERROR:
				reportService -> insertReport(id, MSG_ERR, "Timeout");
